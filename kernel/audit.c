...
static int audit_set_kenny_loggings(struct sk_buff *skb, u32 state)
{
	int rc, ret, i;
	...
	rc = audit_do_config_change("kenny_loggings", &kenny_loggings, state);
	if (state != KL_OFF) {

		/* First time activating Kenny Loggings */
		if (siphash_key_is_zero(&first_key)) {
			...
			audit_send_reply(skb, seq, AUDIT_KL_FIRST_KEY, 0, 0, &first_key, key_len);
...
static int audit_receive_msg(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	...
	case AUDIT_GET: {
		...
		s.kenny_loggings           = kenny_loggings;
	...
	case AUDIT_SET: {
		...
		if (s.mask == AUDIT_STATUS_KENNY_LOGGINGS) {
			err = audit_set_kenny_loggings(skb, s.kenny_loggings);
			if (err < 0)
				return err;
		}
		break;
...
/**
 * audit_log_end - end one audit record with an integrity proof
 * @ab: the audit_buffer
 *
...
void audit_log_end(struct audit_buffer *ab)
{
	...
	if (audit_rate_check()) {
		...
		/* Check feature is enabled and that the given audit record is not of type AUDIT_EOE */
		if (kenny_loggings != KL_OFF && nlh->nlmsg_type != AUDIT_EOE) {
